...
server.get("/login", (req, res) => {
  const scope =
    "user-read-private user-read-email user-top-read playlist-modify-public playlist-read-private playlist-modify-private";
...
server.post("/playlist/add", (req, res) => {
  const {playlistAdd} = req.body.result[1];
  const {trackAdd} = req.body.result[0];
  if (!playlistAdd || !trackAdd) {
    return res.status(400).send({ status: "failed" });
  }
  
  SpotifyUserToken.addTracksToPlaylist(req.body.playlist,"spotify:track:"+req.body.track).then(function(data) {
